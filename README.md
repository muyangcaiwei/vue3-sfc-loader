...
if you want to get .vue file's json, please add `parseJson: true` in the options,
if you want to get the component object from above json, please add `parseComponent: your jsonstring` in the options.
if you set both the parseJson and parseComponent tag, the former one will validate only

# while i compiling the code
node version： 16.x
pc infos: Ubuntu 20.04.3 LTS, 16G RAM

command： npm run build --max_old_space_size=8192 (node will only use 4g memory default in my pc)
