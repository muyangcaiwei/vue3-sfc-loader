...
// import vueLoaderToJs from './vue-loader/index'

/**
 * @internal
 */

export async function parseJsonVue2(source : string, filename : AbstractPath, options : Options) : Promise<ModuleExport> {

	// not recursive
	options.parseComponent = undefined;
	...
	console.log(compileTemplateOptions)
	...
	if ( descriptor.script ) {
		...
		//console.log(interopRequireDefault(createCJSModule(filename, transformedScriptSource, options).exports).default)
		// Object.assign(component, interopRequireDefault(createCJSModule(filename, transformedScriptSource, options).exports).default);
		Object.assign(component, {_script: transformedScriptSource})
	}
	...
	if ( descriptor.template !== null ) {
		...
		Object.assign(component, {_template: templateTransformedSource})
	}
	console.log(component)
	...
	for ( const descStyle of descriptor.styles ) {
		...
		const style = await withCache(compiledCache, [ componentHash, src, descStyle.lang ], async ({ preventCache }) => {
			...
			console.log(compiledStyle.code)
			return compiledStyle.code;
		});

		Object.assign(component, {_style: style})
		Object.assign(component, {_styleScoped: descStyle.scoped ? scopeId : undefined})
...
export async function parseComponentVue2(source : string, filename : AbstractPath, options : Options) : Promise<ModuleExport> {

	const { parseComponent, addStyle } = options 
	
	if (parseComponent == undefined) {
		return undefined
	}
	...
	if (_scopeId != undefined) {
		Object.assign(component, {_scopeId: _scopeId});
	}

	console.log(component)

	if (_script != undefined) {
		Object.assign(component, interopRequireDefault(createCJSModule(filename, _script, options).exports).default);
	}
	
	console.log(component)

	if (_template != undefined) {
		Object.assign(component, createCJSModule(filename, _template, options).exports);
	}
	
	console.log(component)
	if (_style != undefined) {
		addStyle(_style, _styleScoped? _scopeId : undefined);
	}
