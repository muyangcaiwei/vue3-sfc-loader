...
import { parseJsonVue2, parseComponentVue2 } from './parseVue2SFCModule'
 ...
 */
 async function defaultHandleModule(type : string, getContentData : File['getContentData'], path : AbstractPath, options : Options) : Promise<ModuleExport | null> {
	...
	const {parseComponent, parseJson} = options
	console.log(parseComponent)

	if ( parseJson !== undefined) { // parse .vue to json
		return  parseJsonVue2( (await getContentData(false)) as string, path, options)	
	}else if (parseComponent !== undefined) { // parse json to component
		return parseComponentVue2(null, path, options)
	}
